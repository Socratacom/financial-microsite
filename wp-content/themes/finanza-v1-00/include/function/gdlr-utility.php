<?php
	...
	*/
	...
	// escape content
	if( !function_exists('gdlr_escape_content') ){
		function gdlr_escape_content($string){
			return apply_filters('gdlr_escape_content', $string);
		}
	}	
	
	// get sidebar id from name
	if( !function_exists('gdlr_get_sidebar_id') ){
		function gdlr_get_sidebar_id($sidebar_name){
	...
	if( !function_exists('gdlr_content_filter') ){
		...
		add_filter( 'gdlr_the_content', 'do_shortcode'       );
