	...
	*/
	...
	if( !function_exists('gdlr_get_blog_item') ){
		function gdlr_get_blog_item( $settings = array() ){
			...
			$args['offset'] = (empty($settings['offset']))? 0: ((intval($args['paged']) - 1) * intval($args['posts_per_page'])) + $settings['offset'];				
	...
	if( !function_exists('gdlr_get_blog_info') ){
		function gdlr_get_blog_info( $array = array(), $wrapper = true, $sep = '', $style = 'icon' ){
			...
			if( !empty($theme_option['new-fontawesome']) && $theme_option['new-fontawesome'] == 'enable' ){
				$icon_class = array('date'=>'fa fa-clock-o', 'tag'=>'fa fa-tag', 'category'=>'fa fa-folder-o', 
					'comment'=>'fa fa-comments-o', 'author'=>'fa fa-user');
			}else{
				$icon_class = array('date'=>'icon-time', 'tag'=>'icon-tag', 'category'=>'icon-folder-close-alt', 
					'comment'=>'icon-comment-alt', 'author'=>'icon-user');
			}
			...
			foreach($array as $post_info){
				...
				switch( $post_info ){
					case 'date':
						...
						if($style == 'icon'){
							$ret .= '<i class="' . $icon_class['date'] . '"></i>';
						}else{
							$ret .= '<span class="gdlr-head">' . __('Date', 'gdlr_translate') . '</span>';
						}
					...
					case 'category':
						...
						if($style == 'icon'){
							$ret .= '<i class="' . $icon_class['category'] . '"></i>';
						}else{
							$ret .= '<span class="gdlr-head">' . __('Categories', 'gdlr_translate') . '</span>';
						}
						$ret .= $category;					
						$ret .= '</div>';					
						break;
					case 'comment':
						...
						$comment_text = (get_comments_number() > 1)? __('Comments', 'gdlr_translate'): __('Comment', 'gdlr_translate');

						$ret .= '<div class="blog-info blog-comment">';
						if($style == 'icon'){
							$ret .= '<i class="' . $icon_class['comment'] . '"></i>';
						}else{
							$ret .= '<span class="gdlr-head">' . $comment_text . '</span>';
						}
						$ret .= '<a href="' . get_permalink() . '#respond" >' . get_comments_number() . ($style=='icon'? ' ' . $comment_text:'') . '</a>';						
						$ret .= '</div>';						
						break;
					case 'author':
						ob_start();
						...
						if( !empty($sep) ) $ret .= $sep;
						$ret .= '<div class="blog-info blog-author">';
						if($style == 'icon'){
							$ret .= '<i class="' . $icon_class['author'] . '"></i>';
						}else{
							$ret .= '<span class="gdlr-head">' . __('Posted By', 'gdlr_translate') . '</span>';
						}
						$ret .= $author;
						$ret .= '</div>';						
						break;						
	...
	if( !function_exists('gdlr_get_blog_widget') ){
		function gdlr_get_blog_widget($query, $size){
			...
			while($query->have_posts()){ $query->the_post();
				if( $current_size % $size == 0 && $current_size != 0 ){
