...
add_action( 'widgets_init', 'gdlr_recent_portfolio2_widget' );
if( !function_exists('gdlr_recent_portfolio2_widget') ){
	function gdlr_recent_portfolio2_widget() {
		register_widget( 'Goodlayers_Recent_Portfolio2' );
	}
}

if( !class_exists('Goodlayers_Recent_Portfolio2') ){
	class Goodlayers_Recent_Portfolio2 extends WP_Widget{
		...
		function form( $instance ) {
			...
			<p>
				...
				foreach($category_list as $cat_slug => $cat_name){ ?>
					<option value="<?php echo gdlr_escape_content($cat_slug); ?>" <?php if ($category == $cat_slug) echo ' selected '; ?>><?php echo gdlr_escape_content($cat_name); ?></option>				
				<?php } ?>	
				</select> 
			</p>
			...
			<p>
				...
				<input class="widefat" id="<?php echo $this->get_field_id('num_fetch'); ?>" name="<?php echo $this->get_field_name('num_fetch'); ?>" type="text" value="<?php echo gdlr_escape_content($num_fetch); ?>" />
			</p>
